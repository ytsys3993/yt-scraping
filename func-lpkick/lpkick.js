...
// クリックエラーが出ないように解消
// 重い時は240でもOK。軽いときは230
// 250も落ちた(うまくいったときもある？)
// 255は重めでもNG

// 同時起動OK。Excelでアドレス管理。現状は10個(aws6/bk2/sv2)
// ploxyしないとアク禁になるので注意
...
module.exports = (req) => !(async () => {
  try {
    ...
    if (req) {
      ...
      logInfoExp(`キック：${JSON.stringify(conf)}`);
    }
    ...
    if (conf.notCheckOnly) {
      // 指定時刻の15分前まで待つ
      const _now = moment();
      const _set = moment(conf.loginTime).set('seconds', 0).set('milliseconds', 0);
      if (_now.isBefore(_set)) {
        logInfoExp(`${_set.format("YYYY-MM-DD HH:mm:ss")}までログイン処理待機中...`);
      } else {
        logInfoExp(`${_set.format("YYYY-MM-DD HH:mm:ss")}にすでに到達`);
      }
      await awaitTime(_set.diff(_now));
      logInfoExp("ログイン処理想定時間到達");
...
// アクセス集中チェック
async function accessJamCheck(page) {
  try {
    let resultSelectorErr = true;
    let isEndTime = false;
    let spaceMinutes = 5;
    ...
    for (let i = 1; i <= 60; i++) {
      ...
      resultSelectorErr = await page.$('#info');
      if (resultSelectorErr) {
        let msg = await (await resultSelectorErr.getProperty('textContent')).jsonValue()
        if (msg === 'アクセスが集中しています') {
          logInfoExp("アクセス集中のためリロード待ち");
        }  
      }

      // 成功もしくは60回チャレンジして無理だったら強制終了
      if (!resultSelectorErr || tryCnt >= 60) {
        break;
      }
    ...
    if (!resultSelectorErr && tryCnt === 0) {
      logger.info(`アクセス集中なし`);
    }

    if (!resultSelectorErr && tryCnt > 0) {
      logger.info(`アクセス集中解消(${tryCnt}回目に成功)`);
    }

    return !resultSelectorErr;
  } catch (e) {
    logger.error(e)
    await page.screenshot({ path: 'log\\error.png' });
...
async function newBrowser() {
  try {
    ...
    if (conf.isProxy) {
      const proxyName = `--proxy-server=${conf.proxy}`;
      launchOption.args.push(proxyName);
    }
    ...
    // isConnetc=true かつ isProxy=false ならchromeで、それ以外はchrominium
    if (!conf.isProxy && conf.isConnect) {
      const chromeLaunchOption = {}
      if (!conf.isBrowse) {
        chromeLaunchOption.chromeFlags = ['--headless']
      }
      const chrome = await chromeLauncher.launch(chromeLaunchOption);
      const response = await axios.get(`http://localhost:${chrome.port}/json/version`);
      const { webSocketDebuggerUrl } = response.data;
      launchOption.browserWSEndpoint = webSocketDebuggerUrl,
      browser = await puppeteer.connect(launchOption);
    } else {
    ...
    if (conf.proxy) {
      await page.authenticate({
        username: conf.proxyUser,
        password: conf.proxyPass,
      });
      // await page.setExtraHTTPHeaders({
      //   'Proxy-Authorization': 'Basic ' + Buffer.from(`${conf.proxyUser}:${conf.proxyPass}`).toString('base64'),
      // });
      // console.log('test');
      // await awaitTime(30000);
    ...
    page.on('request', (request) => {
      ...
      requestOkUrls.forEach((okUrl) => {
        if (!request.url().startsWith(okUrl)) {
          abortCheck = true;
        }
      })

      if (['image', 'font'].indexOf(request.resourceType()) !== -1 || abortCheck) {
          request.abort();
      } else {
          request.continue();
      }
    ...
    // アクセス集中チェック
    // ※アクセス集中時に、コメントを外し、問題ないかをチェック
    // await accessJamCheck(page);
    
    logInfoExp("ログインページに移動完了");
    ...
    // オリジナルクッキー技
    if (conf.isOrigin) {
      // ブラウザバック用に別のページに最初にアクセスさせておく
      await page.goto('https://t.livepocket.jp/', { waitUntil: 'networkidle2' });
      
      // その上で対象ページに進んでおく
      await page.goto(conf.targetUrl, { waitUntil: 'networkidle2' });

      // disabled外し＆枚数選択
      await targetPageBuySelect(page, 'select');
    }
    ...
    if (!conf.notCheckOnly) {
      ...
      if (conf.isOrigin) {
        // 指定時刻の30秒前まで待つ
        _now = moment();
        _set = moment(conf.startTime).set('seconds', -30).set('milliseconds', conf.advSec);
        if (_now.isBefore(_set)) {
          logInfoExp(`${_set.format("YYYY-MM-DD HH:mm:ss")}までページバック待機中...`);
        } else {
          logInfoExp(`${_set.format("YYYY-MM-DD HH:mm")}にすでに到達`);
        }
        await awaitTime(_set.diff(_now));
      ...
      // 指定時刻まで待つ
      _now = moment();
      _set = moment(conf.startTime).set('seconds', 0).set('milliseconds', conf.advSec);
      if (_now.isBefore(_set)) {
        logInfoExp(`${_set.format("YYYY-MM-DD HH:mm:ss")}まで購入処理待機中...`);
      } else {
        logInfoExp(`${_set.format("YYYY-MM-DD HH:mm")}にすでに到達`);
      }
      await awaitTime(_set.diff(_now));
      logInfoExp("購入処理想定時間到達");

      // 時間カウント開始
      logInfoExp("<成績時間カウント開始>");
      countTime = moment();

      // 購入可能状態確認
      for (let i = 1; i <= 100; i++) {
        logInfoExp(`購入可能状態確認start(${i}回目)`);
        
        // オリジナルクッキー技
        if (i === 1 && conf.isOrigin) {
          logInfoExp('次ページへ移動');
      ...
      logInfoExp("購入可能状態確認end");

      // 枚数選択
      let tryCnt = 0;
      let maxTryCnt = conf.jamTryCnt;
      for (let i = 1; i <= maxTryCnt; i++) {    
        tryCnt = 1;    
        logInfoExp(`枚数選択start(${i}回目)`);
        if (conf.isOrigin && tryCnt === 1) {
          await targetPageBuySelect(page, 'submit');
          if (conf.isOrigin && conf.isOriginCancel) {
            return true;
          }
        } else {
          await targetPageBuySelect(page, 'all');
        }
        logInfoExp(`枚数選択end(${i}回目)`);
        const resultSelectorErr = await page.$('span#remain_sec');
        const resultSelectorNav = await page.$('#navigation > div > p');
        let resultSelectorNavErr = false;
        if (resultSelectorNav) {
          let msg = await (await resultSelectorNav.getProperty('textContent')).jsonValue()
          if (msg === '現在アクセスが集中しています。再度お試しください。') {
            resultSelectorNavErr = true;
            logInfoExp(msg);
          }  
        }
        ...
        if (resultSelectorErr) {
          await jamErrorTry(page);
        } else if (resultSelectorNavErr) {
          // 再選択させる
        } else {
          // ページ遷移先の情報を期待(有償 or 無償 or 再ログイン)
          let resultSelectorKessai = false;
          if (conf.isMobile) {
            resultSelectorKessai = await page.waitForSelector('.form-flat > .event-entry > .payment > .clearfix > .btn-off, #agreement_check_lp, #password', { timeout: 0})
          } else {
            resultSelectorKessai = await page.waitForSelector('.base-wrapper #other_payment_method_select_img, .buy-conf #receive_info, #password', { timeout: 0 })          
          }
          const resultPasswordErr = await page.$('#password');
          if (resultPasswordErr) {
            logger.error("resultPasswordErr")
          }
          if (resultSelectorKessai) {
            break;
          }
        }
      }
      ...
      if (tryCnt === maxTryCnt) {
        // 失敗
        logInfoExp(`混線リピート上限(${maxTryCnt}回)に達したため強制ストップ`)
        return false; 
      }

      logInfoExp('決済処理ページ遷移済');
      
      // 決済処理
      let wasBuyEnd = false;
      for (let i = 1; i <= 100; i++) {  
        logInfoExp(`決済処理start(${i}回目)`);    
      ...
      if (conf.isTest) {
        ...
        if (wasBuyEnd) {
          logInfoExp("決済処理end");    
          logger.info(`<現在時刻：${moment()}／成績：${moment().diff(countTime) / 1000}秒>`);  
          logInfoExp(`<成績：${moment().diff(countTime) / 1000}秒>`);
        } else {
          logInfoExp("決済処理失敗？");
        }
...
async function newLogin(page) {
  try {
    ...
    // ユーザ名、パスワード入力
    if (conf.isTest) {
      await page.type('#email', conf.testUser);
      await page.type('#password', conf.testPass);  
    } else {
    ...
    if (conf.isMobile) {
      ...
      // 検証3
      // logInfoExp("クリックボタン検知前")
      // await page.waitForSelector('#form > nav > ul > li:nth-child(1) > a', { timeout: 0 })
      // logInfoExp("クリックボタン検知")
...
async function checkTargetPage(page) {
  try {
    ...
    if (isTargetPageChecked) {
      logInfoExp(`「${conf.targetName}」の存在を確認(${conf.targetTicket})`);
    }
...
async function moveTargetPage(page) {
  try {
    ...
    if (wasLoadingSucceeded) {
      logInfoExp(`購入状態確認(${tryCnt}回目に成功)`);
    }
...
// 購入処理-枚数選択
// option all or select or submit
async function targetPageBuySelect(page, option) {
  try {

    let cnt = 0;
    const targetSelectArray = [];

    // ノーマル or ロック外しのタイミングで枚数選択
    if (option === 'all' || option === 'select') {
      conf.targetTicket.split(',').forEach(async (targetTicket) => {
        if (option === 'select') {
          // disabledを外す
          logInfoExp(`${targetTicket}ロック外し開始`)
          targetSelectArray.push(page.$eval(targetTicket, el => el.disabled = false));
        }
        // targetSelectArray.push(page.waitForSelector(targetTicket, { timeout: 0 }));
        if (targetSelectArray.length === 0) {
          // 1回はクリック処理を通らないとsubmitが有効にならない
          targetSelectArray.push(page.click(targetTicket));
        }  
        targetSelectArray.push(page.select(targetTicket, conf.targetNumber.split(',')[cnt]))
        cnt++;
      });
      await Promise.all(targetSelectArray);
      logInfoExp(`${cnt}セット選択終了`)
    }

    if (option === 'all') {
      // ※このパートも決済処理onlyでいけるならそっちでやりたい。現状は0.8秒待たないとエラーになりえる
      // 0.8秒待つ (必須。0.5秒out/0.8秒safe?)
    ...
    if (option === 'submit') {
      ...
      // 決済処理only
      let nextPageCheck = false;
      let tryCnt = 0;
      for (let i = 1; i <= 200; i++) {
        tryCnt = i;
        logInfoExp(`購入クリック処理${tryCnt}回目`)
        
        // うまく動いてない時がありそうなのでクリック処理に変更
        // await page.evaluate(() => { 
        //   return buy_ticket();
        // });

        const clickTarget = page.$('#submit > p > button');
        if (clickTarget) {
          const clickSelector = '#submit > p > button';
          // clickpage.click('#submit > p > button');
          await page.evaluate((clickSelector) => document.querySelector(clickSelector).click(), clickSelector)
        }
        resultSelectorErr = page.$('span#remain_sec');
      
        // まだ現在ページじゃなかったら
        nextPageCheck = !page.$('#submit > p > button');
      }
      if (!nextPageCheck) {
        logInfoExp('枚数選択後購入クリックに失敗')        
        return false;
      } else {
        logInfoExp(`決済ページ遷移確認${tryCnt}回目に成功`)
        return true;
      }
    }
...
async function targetPageBuyConfirm(page) {
  try {
    ...
    logInfoExp(`購入処理確定開始(freeBuy:${conf.isFree})`);
    let wasBuy = null;
    if (conf.isFree) {
...
async function targetPayment(page) {
  try {
    ...
    if (!conf.isTest) {
      ...
      logInfoExp("決済ボタンクリック");
    
    } else {
      logInfoExp("【テスト:決済ロジック】");
      return true;
    }
    return true;
  } catch (e) {
