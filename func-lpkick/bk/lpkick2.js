    ...
    if (!conf.notCheckOnly) {
      ...
      for (let i = 1; i <= 100; i++) {        
        ...
        if (resultSelectorErr) {
          ...
          // ページ遷移先の情報を期待(枚数選択 or 有償 or 無償 or error or 再ログイン)
          await page.waitForSelector('.buy_ticket_info_area > #purchase_form > #submit > .btn-procedure-pc-only > .register_input_submit, .base-wrapper #other_payment_method_select_img, .buy-conf #receive_info, #password', { timeout: 0 })          
          const resultReSelector = await page.$('.buy_ticket_info_area > #purchase_form > #submit > .btn-procedure-pc-only > .register_input_submit');
          const resultPasswordErr = await page.$('#password');
          if (!resultReSelector) {
            if (resultPasswordErr) {
              logger.error("resultPasswordErr")
...
async function targetPageBuySelect(page) {
  try {
    ...
    await Promise.all([
      ...
      await page.click('.buy_ticket_info_area > #purchase_form > #submit > .btn-procedure-pc-only > .register_input_submit')
    ]);
    ...
    if (!conf.isTest) {
      ...
      if (!conf.isCredit) {
        ...
        await Promise.all([
          ...
          // https://qiita.com/monaka_ben_mezd/items/4cb6191458b2d7af0cf7
          page.waitForNavigation({waitUntil: ['load', 'networkidle2']}),
          await page.click('.submit > .clearfix > #submit-btn > button > .agreement-text')
        ]);
      } else {
        // 未検証
